...
def _load_clusters() -> Dict[str, Dict[str, Optional[str]]]:
    """
    ...
    existing single-cluster setups (e.g. the Claude Code plugin's
    .mcp.json) keep working unchanged.
    """
    aliases = [a.strip() for a in os.getenv("DOMINO_CLUSTERS", "").split(",") if a.strip()]

    if not aliases:
        host = os.getenv("DOMINO_HOST")
        if not host:
            return {}
        return {"default": {"host": host.rstrip("/"), "api_key": os.getenv("DOMINO_API_KEY")}}

    clusters: Dict[str, Dict[str, Optional[str]]] = {}
    for alias in aliases:
        env_key = re.sub(r"[^A-Za-z0-9]", "_", alias).upper()
        host = os.getenv(f"DOMINO_HOST_{env_key}")
        if not host:
            raise ValueError(
                f"DOMINO_CLUSTERS includes '{alias}' but DOMINO_HOST_{env_key} is not set."
            )
        clusters[alias] = {
            "host": host.rstrip("/"),
            "api_key": os.getenv(f"DOMINO_API_KEY_{env_key}"),
        }
    return clusters


def _default_cluster_alias(clusters: Dict[str, Dict[str, Optional[str]]]) -> Optional[str]:
    override = os.getenv("DOMINO_DEFAULT_CLUSTER")
    if override and override in clusters:
        return override
    return next(iter(clusters), None)


def _resolve_cluster(cluster: Optional[str]) -> Dict[str, Optional[str]]:
    clusters = _load_clusters()
    if not clusters:
        raise ValueError(
            "No Domino cluster configured. Set DOMINO_CLUSTERS plus "
            "DOMINO_HOST_<ALIAS>/DOMINO_API_KEY_<ALIAS> per cluster "
            "(or the legacy DOMINO_HOST/DOMINO_API_KEY for a single cluster)."
        )
    alias = cluster or _default_cluster_alias(clusters)
    if alias not in clusters:
        raise ValueError(f"Unknown cluster '{cluster}'. Configured clusters: {', '.join(clusters)}")
    return clusters[alias]


def _get_domino_host(cluster: Optional[str] = None) -> str:
    """
    Return the base URL for Domino API calls.

    Inside a Domino workspace we route through the local proxy at
    localhost:8899, which always talks to that workspace's own cluster —
    `cluster` is ignored there, since there's only one. Outside (laptop),
    `cluster` selects which configured cluster to use (see
    list_domino_clusters); omitting it uses the default cluster.
    """
    if _is_domino_workspace():
        return "http://localhost:8899"
    return _resolve_cluster(cluster)["host"]


def _get_external_host(cluster: Optional[str] = None) -> str:
    """
    ...
    if not _is_domino_workspace():
        return _get_domino_host(cluster)
...
def _get_auth_headers(cluster: Optional[str] = None) -> dict:
    """
    ...
    Outside Domino, `cluster` selects which configured cluster's API key to use.
    """
    ...
    if not api_key:
        raise ValueError(f"No DOMINO_API_KEY configured for cluster '{cluster or _default_cluster_alias(_load_clusters())}'.")
...
def _extract_and_format_mlflow_url(text: str, user_name: str, project_name: str, cluster: Optional[str] = None) -> str | None:
    """
    ...
    if match:
        ...
        new_url = f"{_get_external_host(cluster)}/experiments/{user_name}/{project_name}/{experiment_id}/{run_id}"
        return new_url
    else:
        return None # Return None if the pattern is not found

def _get_project_id(user_name: str, project_name: str, cluster: Optional[str] = None) -> str | None:
    """
    ...
    headers = {**_get_auth_headers(cluster), "Content-Type": "application/json"}

    url = f"{_get_domino_host(cluster)}/v4/gateway/projects"
...
@mcp.tool()
def list_domino_clusters() -> Dict[str, Any]:
    """
    Lists the Domino clusters configured on this machine via DOMINO_CLUSTERS
    and per-cluster DOMINO_HOST_<ALIAS>/DOMINO_API_KEY_<ALIAS> env vars.

    Every other tool takes an optional `cluster` argument matching one of the
    aliases returned here; omit it to use the default cluster. Call this
    first if you don't already know which cluster alias to pass.
    """
    if _is_domino_workspace():
        return {
            "inside_domino_workspace": True,
            "note": "Running inside a workspace; cluster selection doesn't apply here — this always talks to this workspace's own platform.",
        }
    clusters = _load_clusters()
    return {
...
@mcp.tool()
async def check_domino_job_run_results(user_name: str, project_name: str, run_id: str, cluster: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    Args:
        ...
        cluster (str, optional): Which configured Domino cluster to use (see list_domino_clusters). Defaults to the default cluster.
    """
    ...
    api_url = f"{_get_domino_host(cluster)}/v1/projects/{encoded_user_name}/{encoded_project_name}/run/{encoded_run_id}/stdout"
    headers = _get_auth_headers(cluster)
    try:
        ...
        mlflow_url = _extract_and_format_mlflow_url(initially_filtered_stdout, user_name, project_name, cluster)
...
@mcp.tool()
async def check_domino_job_run_status(user_name: str, project_name: str, run_id: str, cluster: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    api_url = f"{_get_domino_host(cluster)}/v1/projects/{encoded_user_name}/{encoded_project_name}/runs/{encoded_run_id}"
    headers = _get_auth_headers(cluster)
    try:
        response = requests.get(api_url, headers=headers)
        response.raise_for_status()  # Raise an exception for bad status codes (4xx or 5xx)
...
@mcp.tool()
async def run_domino_job(user_name: str, project_name: str, run_command: str, title: str, cluster: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    api_url = f"{_get_domino_host(cluster)}/v1/projects/{encoded_user_name}/{encoded_project_name}/runs"
...
@mcp.tool()
async def get_domino_environment_info(cluster: Optional[str] = None) -> Dict[str, Any]:
    """
    ...
    Args:
        cluster (str, optional): Which configured Domino cluster to use (see list_domino_clusters). Defaults to the default cluster. Ignored when running inside a workspace.
    """
    import subprocess

    info: Dict[str, Any] = {
        "inside_domino_workspace": _is_domino_workspace(),
        "domino_host": _get_domino_host(cluster),
    }
    ...
    if _is_domino_workspace():
        ...
        info["cluster"] = cluster or _default_cluster_alias(_load_clusters())
        info["note"] = (
...
def _get_remote_file_info(user_name: str, project_name: str, file_path: str, cluster: Optional[str] = None) -> Dict[str, Any] | None:
    """
    ...
    url = f"{_get_domino_host(cluster)}/v4/files/browseFiles"
    params = {
...
@mcp.tool()
async def list_domino_project_files(user_name: str, project_name: str, path: str = "/", cluster: Optional[str] = None) -> Dict[str, Any]:
    """
...
async def upload_file_to_domino_project(
    ...
    url = f"{_get_domino_host(cluster)}/v1/projects/{encoded_user_name}/{encoded_project_name}/{file_path}"
...
async def download_file_from_domino_project(
    ...
    """
    headers = {
        **_get_auth_headers(cluster),
        "Content-Type": "application/json",
        "Cache-Control": "no-cache",
    }

    # Use editCode endpoint to get file content
    url = f"{_get_domino_host(cluster)}/v4/files/editCode"
    params = {
        "ownerUsername": user_name,
        "projectName": project_name,
    ...
    try:
        ...
        remote_info = _get_remote_file_info(user_name, project_name, file_path, cluster)
...
async def smart_sync_file(
    ...
    if remote_info is None:
        result = await upload_file_to_domino_project(user_name, project_name, file_path, content, cluster=cluster)
    ...
    if cached_version is None and not force_overwrite:
        ...
        remote_result = await download_file_from_domino_project(user_name, project_name, file_path, cluster=cluster)
