# Contributing to domino-cline-plugin

Thank you for your interest in contributing to the Domino Data Lab plugin for Cline!
...
```bash
git clone https://github.com/YOUR_USERNAME/domino-cline-plugin.git
...
Test the plugin locally with Cline. Skills are installed as symlinks into
`~/.cline/skills/` (global — applies to every project you open in Cline):
...
The MCP server is registered separately (Cline → MCP Servers → add server):
...
Credentials go in `~/.domino/.env` (see README § Install).
...
3. Symlink it so Cline discovers it:
   `ln -sfn "$(pwd)/skills/domino-your-skill" ~/.cline/skills/domino-your-skill`
...
tools with the `@mcp.tool()` decorator. Any tool that targets a specific
cluster takes an optional `cluster` argument (see the `list_domino_clusters`
tool).

### Adding a Hook

Cline runs exactly one executable file per event, named after the event
...
3. Verify MCP tools work (e.g. `list_domino_clusters`)
